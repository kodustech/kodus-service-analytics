...
interface PullRequestSizeMetrics {
  averagePRSize: number;
}

interface PullRequestsByDevChartData {
  weekStart: string;
  author: string;
  prCount: number;
}

interface PullRequestsOpenedVsClosedData {
  weekStart: string;
  openedCount: number;
  closedCount: number;
  ratio: number;
...
interface LeadTimeBreakdownData {
  weekStart: string;
  prCount: number;
...
interface SuggestionCategoryCount {
  category: string;
  count: number;
}

interface SuggestionRepoCount {
  repository: string;
  category: string;
  count: number;
}
...
class BigQueryService {
  ...
  private getTablePath(dataset: keyof typeof DATASETS, table: string) {
